  ...
  const handleLogin = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/auth/login`, {
        ...
        credentials: "include", // ✅ required for HTTP-only cookie to be set
        body: JSON.stringify({
          email: username,
          password: password,
        }),
      ...
      // ✅ No localStorage for token — it's in the HTTP-only cookie now

      if (remember) {
        localStorage.setItem("rememberAdmin", "true"); // fine, not a secret
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 dark:from-slate-900 dark:to-slate-800 flex items-center justify-center px-4">
      ...
      >
          ...
          >
            <img
              ...
              className={`w-6 h-6 opacity-60 dark:invert brightness-10 contrast-150 hover:opacity-100 hover:scale-90 hover:rotate-0 hover:drop-shadow-md hover:-translate-y-0.5 transition-all duration-300 cursor-pointer ${
                showPassword ? "rotate-0 scale-80" : "rotate-0 scale-80"
              }`}
            />
          </button>
        </div>
        ...
        {/* Caps Lock */}
        {capsLock && (
          <p className="text-orange-500 text-sm mb-2">Caps Lock is ON</p>
        )}
